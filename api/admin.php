<?php
session_start();
include 'p2.php'; // Defines $con (cart DB) and $con1 (orders DB)
include 'db_connect.php'; // Defines $conn (users DB)

// Contact Form Database Connection (user1 DB)
$con_mysql = @mysqli_connect(getenv('DB_HOST') ?: "localhost", getenv('DB_USER') ?: "root", getenv('DB_PASSWORD') ?: "");
if ($con_mysql) {
    $db_contact_name = getenv('DB_NAME_CONTACT') ?: "user1";
    @mysqli_query($con_mysql, "CREATE DATABASE IF NOT EXISTS " . $db_contact_name);
    mysqli_close($con_mysql);
}
$con_contact = mysqli_connect(getenv('DB_HOST') ?: "localhost", getenv('DB_USER') ?: "root", getenv('DB_PASSWORD') ?: "", getenv('DB_NAME_CONTACT') ?: "user1");
if (!$con_contact) {
