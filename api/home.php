  ...
  <section class="video-section">
    ...
    <h1 style="text-align: center;margin-bottom: 20px; margin-top: 8px;">New Arrivals</h1>
        <div class="product-container">
            <?php
            $new_arrivals = mysqli_query($con, "SELECT * FROM store_products ORDER BY date DESC, id DESC LIMIT 4");
            while ($p = mysqli_fetch_assoc($new_arrivals)) {
                ...
                $detail_url = "product detail.php?name=" . urlencode($p['name']) . "&price=" . urlencode($p['price']) . "&image=" . urlencode($p['image']);
                ?>
                <div class="product-card" data-id="<?php echo $p['id']; ?>" data-price="<?php echo $p['price']; ?>" data-name="<?php echo htmlspecialchars($p['name']); ?>">
                    <div class="product-image-container">
                        <a href="<?php echo $detail_url; ?>"><img src="<?php echo htmlspecialchars($p['image']); ?>" alt="<?php echo htmlspecialchars($p['name']); ?>"></a>
                        <button class="wishlist-btn" type="button" aria-label="Add to wishlist"><i class="fa-regular fa-heart"></i></button>
                    </div>
                    <div class="product-details">
                        <h3 class="product-title" style="cursor: pointer;" onclick="window.location.href='<?php echo $detail_url; ?>'"><?php echo htmlspecialchars($p['name']); ?></h3>
...
<img src="images/hand.jpg" class="hand-img">
  ...
  <br><br>
    ...
    <section class="trending-section">
        ...
        <div class="product-container">
            <?php
            $top_trending = mysqli_query($con, "SELECT * FROM store_products WHERE featured = 1 ORDER BY sales DESC, id DESC LIMIT 4");
            if (mysqli_num_rows($top_trending) < 4) {
                $top_trending = mysqli_query($con, "SELECT * FROM store_products ORDER BY sales DESC LIMIT 4");
            }
            while ($p = mysqli_fetch_assoc($top_trending)) {
                $formatted_price = number_format($p['price']);
                $orig_price_html = '';
                if (!empty($p['original_price']) && $p['original_price'] > 0) {
                    $orig_price_html = '<span class="original-price">₹' . number_format($p['original_price']) . '</span>';
...
<footer>
  ...
  <script>
    ...
    (function() {
            ...
            checkoutBtn.addEventListener("click", (e) => {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        alert("Failed to sync cart: " + data.message);
        ...
        document.querySelectorAll("form[action='../api/insert_cart.php']").forEach(form => {
